...
from .steps import EchoStep, HttpStep, ForlaAgentStep, TransformStep
from .steps._http import HttpRequestInput, HttpResponseOutput
from .steps.forla_agent import ForlaAgentInput
...
def _create_agent_template():
    """Create a customizable ForlaAgent step template."""
    ...
    return ForlaAgentStep(
        step_id="forla_agent_step",
        metadata=StepMetadata(
            name="ForlaAgent Step",
            description="AI-powered agent using Forla - customize the instructions and model",
            tags=["forla", "agent", "ai"],
        ),
        agent=agent,
    )
...
def create_simple_agent_workflow() -> ComponentModel:
    ...
    # Create ForlaAgent for workflow
    model_client = OpenAIChatCompletionClient(
        model="gpt-4.1-mini",
    ...
    agent_step = ForlaAgentStep(
        step_id="agent_summarize",
        metadata=StepMetadata(
            name="ForlaAgent Summarize",
            description="Summarize content using Forla AI agent",
            tags=["ai", "summarize", "forla"],
        ),
        agent=agent,
    )
