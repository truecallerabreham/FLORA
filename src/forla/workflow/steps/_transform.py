...
class TransformStepConfig(BaseStepConfig):
    ...
    mappings: Dict[str, Any] = Field(
        default_factory=dict, description="Field mappings from input to output"
    )
...
class TransformStep(Component[TransformStepConfig], BaseStep[Any, Any]):
    ...
    component_type = "step"
    component_provider_override = "forla.workflow.TransformStep"
    ...
    def _validate_mappings(self):
        ...
        for out_field, in_field in self.mappings.items():
            if out_field not in output_fields:
                raise ValueError(
                    f"Output field '{out_field}' not in output schema for step {self.step_id}"
                )
            ...
            if isinstance(in_field, str) and not in_field.startswith("static:"):
                if in_field not in input_fields:
                    raise ValueError(
                        f"Input field '{in_field}' not in input schema for step {self.step_id}"
                    )
    ...
    async def execute(self, input_data: BaseModel, context: Context) -> BaseModel:
        ...
        for out_field, in_field in self.mappings.items():
            ...
            coerced_value = coerce_value_to_schema_type(
                raw_value, out_field, output_schema
            )
    ...
    def _to_config(self) -> TransformStepConfig:
        ...
        return TransformStepConfig(**base_data, mappings=self.mappings)
