...
class HttpRequestInput(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(
        default=None, description="Request data for POST/PUT"
    )
...
class HttpStep(
    Component[HttpStepConfig], BaseStep[HttpRequestInput, HttpResponseOutput]
):
    """A step that makes HTTP requests and returns the response."""

    component_config_schema = HttpStepConfig
    component_type = "step"
    component_provider_override = "forla.workflow.HttpStep"
    ...
    async def execute(
        self, input_data: HttpRequestInput, context: Context
    ) -> HttpResponseOutput:
        ...
        try:
            ...
            if input_data.data and input_data.method.upper() in [
                "POST",
                "PUT",
                "PATCH",
            ]:
            ...
            response = requests.request(
                method=input_data.method.upper(), url=input_data.url, **request_kwargs
            )
            ...
            context.set(
                ...
                {"type": "timeout", "message": error_msg, "elapsed_time": elapsed_time},
            )
